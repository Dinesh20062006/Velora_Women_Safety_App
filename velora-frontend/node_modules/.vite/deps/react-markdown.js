...
import { A as unreachable, D as constants, E as types, O as codes, S as markdownSpace, T as values, _ as asciiDigit, a as push, b as markdownLineEnding, c as visit, g as asciiControl, h as asciiAtext, i as combineExtensions, k as ok, l as classifyCharacter, m as asciiAlphanumeric, n as factorySpace, o as splice, p as asciiAlpha, r as resolveAll, s as toString$1, t as blankLine, u as normalizeIdentifier, v as asciiHexDigit, x as markdownLineEndingOrSpace, y as asciiPunctuation } from "./blank-line-BrubXZRg.js";
...
* @type {Tokenizer}
*/
function tokenizeAttention(effects, ok$35) {
...
* @type {Tokenizer}
*/
function tokenizeAutolink(effects, ok$34, nok) {
	let size = 0;
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeBlockQuoteStart(effects, ok$32, nok) {
	const self = this;
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeBlockQuoteContinuation(effects, ok$33, nok) {
	const self = this;
	return contStart;
	/**
	* Start of block quote continuation.
	...
	*/
	function contBefore(code) {
		return effects.attempt(blockQuote, ok$33, nok)(code);
	}
}
/** @type {Exiter} */
...
* @type {Tokenizer}
*/
function tokenizeCharacterEscape(effects, ok$31, nok) {
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeCharacterReference(effects, ok$30, nok) {
	const self = this;
	let size = 0;
	/** @type {number} */
	let max;
...
* @type {Tokenizer}
*/
function tokenizeCodeFenced(effects, ok$27, nok) {
	const self = this;
	/** @type {Construct} */
	const closeStart = {
		partial: true,
	...
	*/
	function infoBefore(code) {
		if (code === codes.eof || markdownLineEnding(code)) {
			...
			return self.interrupt ? ok$27(code) : effects.check(nonLazyContinuation, atNonLazyBreak, after)(code);
		}
	...
	* @type {Tokenizer}
	*/
	function tokenizeCloseStart(effects, ok$28, nok) {
		let size = 0;
		return startBefore;
		/**
		*
...
* @type {Tokenizer}
*/
function tokenizeNonLazyContinuation(effects, ok$29, nok) {
	const self = this;
	return start;
	/**
	*
	...
	*/
	function lineStart(code) {
		return self.parser.lazy[self.now().line] ? nok(code) : ok$29(code);
	}
}
//#endregion
...
* @type {Tokenizer}
*/
function tokenizeCodeIndented(effects, ok$26, nok) {
	const self = this;
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeCodeText(effects, ok$25, nok) {
	const self = this;
	let sizeOpen = 0;
	/** @type {number} */
	let size;
...
* @type {Tokenizer}
*/
function tokenizeContent(effects, ok$23) {
	/** @type {Token | undefined} */
	let previous;
	return chunkStart;
	/**
...
* @type {Tokenizer}
*/
function tokenizeContinuation(effects, ok$24, nok) {
	const self = this;
	...
	*/
	function prefixed(code) {
		...
		ok(self.parser.constructs.disable.null, "expected `disable.null` to be populated");
		const tail = self.events[self.events.length - 1];
		if (!self.parser.constructs.disable.null.includes("codeIndented") && tail && tail[1].type === types.linePrefix && tail[2].sliceSerialize(tail[1], true).length >= constants.tabSize) return ok$24(code);
		return effects.interrupt(self.parser.constructs.flow, nok, ok$24)(code);
	}
}
//#endregion
...
*   Start state.
*/
function factoryLabel(effects, ok$22, nok, type, markerType, stringType) {
	const self = this;
	let size = 0;
	/** @type {boolean} */
	let seen;
...
* @type {Tokenizer}
*/
function tokenizeDefinition(effects, ok$21, nok) {
	const self = this;
	/** @type {string} */
	let identifier;
	return start;
...
* @type {Tokenizer}
*/
function tokenizeHardBreakEscape(effects, ok$20, nok) {
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeHeadingAtx(effects, ok$19, nok) {
	let size = 0;
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeHtmlFlow(effects, ok$17, nok) {
	const self = this;
	/** @type {number} */
	let marker;
	/** @type {boolean} */
	...
	*/
	function open(code) {
		...
		if (code === codes.questionMark) {
			...
			return self.interrupt ? ok$17 : continuationDeclarationInside;
		}
		if (asciiAlpha(code)) {
			ok(code !== null);
			effects.consume(code);
	...
	*/
	function cdataOpenInside(code) {
		...
		if (code === value.charCodeAt(index++)) {
			effects.consume(code);
			if (index === value.length) return self.interrupt ? ok$17 : continuation;
			return cdataOpenInside;
		}
		return nok(code);
	}
	...
	function tagName(code) {
		if (code === codes.eof || code === codes.slash || code === codes.greaterThan || markdownLineEndingOrSpace(code)) {
			...
			if (!slash && !closingTag && htmlRawNames.includes(name)) {
				...
				return self.interrupt ? ok$17(code) : continuation(code);
			}
	...
	*/
	function basicSelfClosing(code) {
		if (code === codes.greaterThan) {
			effects.consume(code);
			return self.interrupt ? ok$17 : continuation;
		}
		return nok(code);
	}
	/**
...
* @type {Tokenizer}
*/
function tokenizeBlankLineBefore(effects, ok$18, nok) {
	return start;
	/**
	...
	*/
	function start(code) {
		...
		effects.consume(code);
		effects.exit(types.lineEnding);
		return effects.attempt(blankLine, ok$18, nok);
	}
}
//#endregion
...
* @type {Tokenizer}
*/
function tokenizeHtmlText(effects, ok$16, nok) {
	const self = this;
...
* @type {Tokenizer}
*/
function tokenizeLabelEnd(effects, ok$12, nok) {
	const self = this;
...
* @type {Tokenizer}
*/
function tokenizeResource(effects, ok$13, nok) {
...
* @type {Tokenizer}
*/
function tokenizeReferenceFull(effects, ok$14, nok) {
	const self = this;
	...
	*/
	function referenceFullAfter(code) {
		return self.parser.defined.includes(normalizeIdentifier(self.sliceSerialize(self.events[self.events.length - 1][1]).slice(1, -1))) ? ok$14(code) : nok(code);
	}
	/**
...
* @type {Tokenizer}
*/
function tokenizeReferenceCollapsed(effects, ok$15, nok) {
...
* @type {Tokenizer}
*/
function tokenizeLabelStartImage(effects, ok$11, nok) {
	const self = this;
	return start;
	/**
	...
	*/
	function after(code) {
		/* c8 ignore next 3 */
		return code === codes.caret && "_hiddenFootnoteSupport" in self.parser.constructs ? nok(code) : ok$11(code);
	}
}
//#endregion
...
* @type {Tokenizer}
*/
function tokenizeLabelStartLink(effects, ok$10, nok) {
	const self = this;
	return start;
	/**
...
* @type {Tokenizer}
*/
function tokenizeLineEnding(effects, ok$9) {
	return start;
	/** @type {State} */
	function start(code) {
		ok(markdownLineEnding(code), "expected eol");
		...
		effects.consume(code);
		effects.exit(types.lineEnding);
		return factorySpace(effects, ok$9, types.linePrefix);
	}
}
//#endregion
...
* @type {Tokenizer}
*/
function tokenizeThematicBreak(effects, ok$8, nok) {
	let size = 0;
	/** @type {NonNullable<Code>} */
	let marker;
	return start;
...
* @type {Tokenizer}
*/
function tokenizeListContinuation(effects, ok$5, nok) {
	const self = this;
	ok(self.containerState, "expected state");
	...
	function onBlank(code) {
		...
		return factorySpace(effects, ok$5, types.listItemIndent, self.containerState.size + 1)(code);
	}
	/** @type {State} */
	function notBlank(code) {
		...
		self.containerState.furtherBlankLines = void 0;
		self.containerState.initialBlankLine = void 0;
		return effects.attempt(indentConstruct, ok$5, notInCurrentItem)(code);
	}
	/** @type {State} */
	function notInCurrentItem(code) {
		...
		ok(self.parser.constructs.disable.null, "expected `disable.null` to be populated");
		return factorySpace(effects, effects.attempt(list$1, ok$5, nok), types.linePrefix, self.parser.constructs.disable.null.includes("codeIndented") ? void 0 : constants.tabSize)(code);
	}
}
/**
* @this {TokenizeContext}
...
* @type {Tokenizer}
*/
function tokenizeIndent(effects, ok$6, nok) {
	const self = this;
	ok(self.containerState, "expected state");
	ok(typeof self.containerState.size === "number", "expected size");
	...
	function afterPrefix(code) {
		ok(self.containerState, "expected state");
		const tail = self.events[self.events.length - 1];
		return tail && tail[1].type === types.listItemIndent && tail[2].sliceSerialize(tail[1], true).length === self.containerState.size ? ok$6(code) : nok(code);
	}
}
/**
* @this {TokenizeContext}
...
* @type {Tokenizer}
*/
function tokenizeListItemPrefixWhitespace(effects, ok$7, nok) {
	const self = this;
	ok(self.parser.constructs.disable.null, "expected `disable.null` to be populated");
	...
	function afterPrefix(code) {
		const tail = self.events[self.events.length - 1];
		return !markdownSpace(code) && tail && tail[1].type === types.listItemPrefixWhitespace ? ok$7(code) : nok(code);
	}
}
//#endregion
...
* @type {Tokenizer}
*/
function tokenizeSetextUnderline(effects, ok$3, nok) {
	const self = this;
	/** @type {NonNullable<Code>} */
	let marker;
	return start;
...
function tokenizeContainer(effects, ok$2, nok) {
	...
	return factorySpace(effects, effects.attempt(this.parser.constructs.document, ok$2, nok), types.linePrefix, this.parser.constructs.disable.null.includes("codeIndented") ? void 0 : constants.tabSize);
}
//#endregion
...
function initializeFlow(effects) {
	const self = this;
	const initial = effects.attempt(blankLine, atBlankEnding, effects.attempt(this.parser.constructs.flowInitial, afterConstruct, factorySpace(effects, effects.attempt(this.parser.constructs.flow, afterConstruct, effects.attempt(content$1, afterConstruct)), types.linePrefix)));
	return initial;
	/** @type {State} */
